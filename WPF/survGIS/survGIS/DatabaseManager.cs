...
namespace survGIS
{
    class DatabaseManager
    {
        ...
        public ObservableCollection<Job> getTable()
        {
            ...
            sqlCommand = string.Format("SELECT JobNo, Town, Page, Block, Lot FROM PCVA");
            command.CommandText = sqlCommand;
            SQLiteDataReader rdr = command.ExecuteReader();

            while (rdr.Read())
            {
                joblist.Add(new Job(rdr.GetInt32(0), 6, GetSafeString(rdr, 2), GetSafeString(rdr, 3), GetSafeString(rdr, 4)));
            }
