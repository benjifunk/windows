...
namespace survGIS
{
    ...
    public partial class MapperView : UserControl
    {
        ...
        {
            InitializeComponent();
            this.DataContext = new MapperViewModel();
            Initialize();
        }
        ...
        public void SelectionChanged(object sender, EventArgs e)
        {
            ...
            if (MyMapView.Map != null)
            {
                ...
                Job newjob = (Job)jobDG.SelectedItem;

                // datagrid.selectionchanged fires when the window is resized, objects loaded, etc so check to see if we've actually changed the selection on the  
                if (newjob != null && curJob != newjob)
                {
                    // Change this when the databases are correctly set up.
                    //
                    _currTaxID = string.Format("{0}.-{1}-{2}", newjob.Sec, newjob.Blk, newjob.Lot);
                    //
                    curJob = newjob;
                }
            }
